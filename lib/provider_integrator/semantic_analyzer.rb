...
module ProviderIntegrator
  ...
  class SemanticAnalyzer
    ...
    def webhook?(endpoint)
      return true if endpoint.path.downcase.include?("webhook") || endpoint.path.downcase.include?("callback")

      # Публичный POST с телом запроса, не подпадающий под создание операции —
      # похоже на входящее уведомление от провайдера
      endpoint.http_method == "post" && endpoint.public? && !endpoint.request_body_schema.nil?
    end
    ...
    def create?(endpoint)
      endpoint.http_method == "post" && !endpoint.request_body_schema.nil? && !cancel?(endpoint) && !webhook?(endpoint)
    end
    ...
    def webhook_source(from_paths, from_section)
      ...
      return :path unless from_paths.empty?
